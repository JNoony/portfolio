...
function (_EventEmitter) {
  ...
  function FormController() {
    ...
    _this.registered = {}; // Map to store fields being removed
  ...
  _createClass(FormController, [{
    ...
    value: function next() {
      ...
      this.validate(); // If its valid move on
    ...
    value: function getAllTouched() {
      debug$2('Gettings touched'); // So we because all fields controll themselves and, "inform", this controller
      // of their changes, we need to literally itterate through all registered fields
    ...
    value: function getErrors() {
      debug$2('Gettings errors'); // So we because all fields controll themselves and, "inform", this controller
      // of their changes, we need to literally itterate through all registered fields
    ...
    key: "register",
    value: function register(name, field) {
      debug$2('Register', name, field.state); // Determine if the field has been registered before

      var registered = this.registered[name]; // Set registered flag

      this.registered[name] = true; // Always register the field

      this.fields.set(name, field); // Example foo.bar.baz[3].baz >>>> foo.bar.baz

      var magicValue = name.slice(0, name.lastIndexOf('[') != -1 ? name.lastIndexOf('[') : name.length); // Always clear out expected removals when a reregistering array field comes in
      ...
      var initialValue = ObjectMap.get(this.options.initialValues, name);
      ...
      var savedState = ObjectMap.get(this.savedValues, name);

      if (field.keepState && savedState) {
        debug$2("Setting field ".concat(name, "'s kept state"), savedState);
        field.fieldApi.setValue(savedState.value);
        field.fieldApi.setTouched(savedState.touched); // Remove the saved state
    ...
    value: function deregister(name) {
      debug$2('Deregister', name);
      var field = this.fields.get(name); // Example foo.bar.baz[3].baz >>>> foo.bar.baz

      var magicValue = name.slice(0, name.lastIndexOf('[') != -1 ? name.lastIndexOf('[') : name.length); // If the fields state is to be kept then save the value
      ...
      this.emit('change'); //this.emit('value', name); // << WHY DID I PUT THIS 
    }
  }, {
    key: "expectRemoval",
...
function useField() {
  ...
  var field = fieldProps.field,
      ...
      userProps = _objectWithoutProperties(fieldProps, ["field", "validate", "validationSchema", "mask", "maskWithCursorOffset", "format", "parse", "initialValue", "validateOnChange", "validateOnBlur", "validateOnMount", "maskOnBlur", "allowEmptyString", "onValueChange", "notify", "keepState", "maintainCursor", "debug", "shadow", "step"]); // Grab the form register context


  var updater = useContext(FormRegisterContext); // Grab the form state

  var formApi = useFormApi(); // Generate validation function

  var validate = generateValidationFunction(validationFunc, validationSchema); // Initialize state 
  ...
  useEffect(function () {
    ...
    updater.update(field, fieldObj); // Should re-trigger validation if validation handler updates
...
var useArrayField = function useArrayField(_ref) {
  ...
  var fullField = formApi.getFullField(field);
  var initialVals = formApi.getInitialValue(fullField) || initialValue || []; // TODO throw error if initial value and its not array
  ...
  var _remove = function remove(i) {
    ...
    updater.expectRemoval("".concat(field)); // Remove the key
